...
function MedicalID() {
  ...
  const handleAddContact = async () => {
    ...
    const updatedContacts = [...medicalData.emergency_contacts, { ...newContact }];
    const updatedData = { ...medicalData, emergency_contacts: updatedContacts };
    
    try {
      const existingData = await getMedicalID();
      if (existingData) {
        await updateMedicalID(updatedData);
      } else {
        await createMedicalID(updatedData);
      }
  ...
  const handleDeleteContact = async (index) => {
    if (window.confirm('Are you sure you want to delete this emergency contact?')) {
      const updatedContacts = medicalData.emergency_contacts.filter((_, i) => i !== index);
      const updatedData = { ...medicalData, emergency_contacts: updatedContacts };
